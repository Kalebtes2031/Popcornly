...
const loadRecommendations = async () => import("../../services/recommendations");

test(
  "fetchAIRecommendations returns [] when endpoint missing and demo disabled",
  { concurrency: false },
  async () => {
    const { fetchAIRecommendations } = await loadRecommendations();
    process.env.EXPO_PUBLIC_RECOMMENDER_ENDPOINT = "";
    process.env.EXPO_PUBLIC_ENABLE_CLIENT_AI_DEMO = "false";
    process.env.EXPO_PUBLIC_OPENAI_API_KEY = "";
...
test(
  "fetchAIRecommendations parses JSON from OpenAI output_text in demo mode",
  { concurrency: false },
  async () => {
    const { fetchAIRecommendations } = await loadRecommendations();
    process.env.EXPO_PUBLIC_RECOMMENDER_ENDPOINT = "";
    process.env.EXPO_PUBLIC_ENABLE_CLIENT_AI_DEMO = "true";
    process.env.EXPO_PUBLIC_OPENAI_API_KEY = "sk-demo";
...
test(
  "fetchRecommendedContent enriches recommendation with TMDB search",
  { concurrency: false },
  async () => {
    const { fetchRecommendedContent } = await loadRecommendations();
    process.env.EXPO_PUBLIC_RECOMMENDER_ENDPOINT = "https://example.test/reco";
    process.env.EXPO_PUBLIC_ENABLE_CLIENT_AI_DEMO = "false";
    process.env.EXPO_PUBLIC_OPENAI_API_KEY = "";
...
test(
  "fetchRecommendedContent chooses higher score for mediaType any",
  { concurrency: false },
  async () => {
    const { fetchRecommendedContent } = await loadRecommendations();
    process.env.EXPO_PUBLIC_RECOMMENDER_ENDPOINT = "https://example.test/reco";
